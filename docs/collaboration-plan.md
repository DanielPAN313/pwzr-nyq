...
- API 契约：小程序调用的 `/api/sports-app/*` 关键接口在本地后端中必须存在
- 数据库契约：场馆、球局、订单、报名、信用、通知和互评字段必须保留
- 演示数据契约：本地首次启动必须有基础场馆、球局、开放时段和演示球队
- 协作文档：自测清单和 UI 合并清单是否存在并覆盖关键步骤
- 演示准备：README、关键文档、环境模板和忽略规则是否齐全
- GitHub 协作：PR 模板和 Actions 是否覆盖功能分支与 UI 分支协作
- 仓库卫生：`.env`、截图、APK、日志、`node_modules` 等本机产物不能被追踪
