...
- 已安全接入启动页，保留品牌动效并自动打开账号页，演示模式可继续快速进首页。
...
- 订单页支持取消/退款规则提示、确认弹窗、退款预留结果和信用影响展示。
- 订单页支持按状态展示下一步提示。
...
- `npm run check` 已覆盖：
  ...
  - UI 结构检查，确保注册页面保留 `.page`、`.section` 和全局核心样式
  - API 契约检查，覆盖小程序调用的 `/api/sports-app/*` 关键接口
  ...
  - 演示/交接准备检查
  - 仓库卫生检查，阻止 `.env`、截图、APK、日志等本机产物入库
  - 部署准备检查
