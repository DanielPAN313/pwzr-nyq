# 宁约球小程序设计系统

## 作用域

这个设计系统用于微信小程序主工程：

```text
miniprogram/
```

旧 `site/` 和 `modules/` 只能作为业务流程、文案和视觉参考，不再作为设计系统的主实现面。

## 设计目标

宁约球应该像一个紧凑、可靠、年轻的校园运动工具，而不是营销页。用户进入后要能快速完成找场、订场、找球局、报名、支付占位、核销、查看信用分等操作。

视觉气质：

- 年轻、运动、绿色、可信
- 接近微信小程序原生交互
- 信息密度适中，手机上易扫读
- 场馆、订单、核销、信用状态要清晰

## 色彩

```css
page {
  background: #f4f7f2;
  color: #102016;
}

.primary {
  background: #12824f;
  color: #ffffff;
}
```

核心色：

- 背景：`#f4f7f2`
- 卡片：`#ffffff`
- 主文字：`#102016`
- 弱文字：`#6f7c72`
- 主绿色：`#12824f`
- 深绿色：`#0b6b3e`
- 边框：`#dbe4d8`
- 提醒橙：`#f07a32`
- 风险红：`#cf2e2e`

## 布局

- 页面统一使用 `.page` 容器，左右留 `24rpx`。
- 模块块面使用 `.section`，只承载明确的一组信息。
- 卡片用于场馆、球局、订单、消息这类重复实体。
- 不要卡片套卡片。
- 列表行优先清晰可扫，不要做成过大的宣传块。
- 底部 tab 页面要给内容留足安全区空间。
- 多数页面首屏说明区使用深绿渐变标题区，标题、说明和主要入口必须在内容层上方。
- 详情页、表单页、管理页优先采用“标题说明 -> 核心内容 -> 操作按钮”的纵向结构。
- 长列表和工作台页面要保持信息密度，避免把每条业务数据做成过大的装饰卡。

## 组件规则

- 主操作按钮使用绿色。
- 支付、提醒、补缴类操作可以使用橙色。
- 取消、封禁、删除类操作使用红色。
- 状态标签要短：如 `可订`、`缺2人`、`待支付`、`已核销`。
- loading、error、empty 三种状态要使用统一的间距、弱文字和操作按钮，不要每个页面单独做一套视觉。
- 筛选项尽量用短中文，允许换行，不能横向溢出。
- 表单 label 放在输入项上方或左侧，错误提示靠近字段。
- 启动页和首屏标题区可以保留低透明光斑，但只能作为背景层，不能覆盖文字、按钮、表单、列表、底部 tab 或任何可点击区域。
- 光斑、球场线、品牌动效只放在启动页或非功能标题区。不要把强动效、3D 或装饰光效加到按钮、订单、场馆卡、筛选、核销、支付等功能控件上。
- 按钮、状态标签、输入框和 picker 要有稳定高度，避免微信开发者工具缩放或 Mac/Windows 显示差异导致文字挤压。

## 小程序实现约束

- 使用 WXML/WXSS/JS，不使用浏览器 DOM API。
- 请求使用 `wx.request`，路由使用 `wx.navigateTo` 或 `wx.switchTab`。
- 本地缓存使用 `wx.setStorageSync` / `wx.getStorageSync`。
- 不要在小程序页面里使用 `window`、`document`、`localStorage`、`fetch`。
- 新增 tab 页面后必须同时更新 `miniprogram/app.json` 的 `pages` 和 `tabBar.list`。

## 检查

提交前运行：

```bash
npm run check
```

这个命令会检查页面注册、页面四件套、JSON 语法、UTF-8 编码、浏览器 API 误用和 UI 基础结构。注册页面必须保留 `.page`、`.section` 和全局核心样式。
